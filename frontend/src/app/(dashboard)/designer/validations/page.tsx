...
export default function DesignerValidationsPage() {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { isDesigner, isLoadingUser } = useAuth();
  ...
  useEffect(() => {
    if (!isLoadingUser && !isDesigner) {
      router.push("/");
    }
  }, [isLoadingUser, isDesigner, router]);

  const { data: validationsData, isLoading } = useQuery({
    queryKey: ["designerValidations", page, statusFilter],
    queryFn: async () => {
      ...
      const response = await adminApi.getDesignerValidations(params);
  ...
  const approveMutation = useMutation({
    mutationFn: (orderId: string) => adminApi.approveDesignerValidation(orderId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["designerValidations"] });
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ orderId, comment }: { orderId: string; comment: string }) =>
      adminApi.rejectDesignerValidation(orderId, comment),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["designerValidations"] });
  ...
  const validations: ValidationItem[] = validationsData?.items ?? [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-foreground">اعتبارسنجی سفارشات</h1>
      ...
      <Modal
        ...
        onClose={() => { setShowRejectModal(false); setRejectComment(""); }}
        title="رد اعتبارسنجی"
        size="md"
      >
        <div className="space-y-4">
            ...
            <Button
              ...
              onClick={() => {
                if (selectedValidation) {
                  rejectMutation.mutate({ orderId: selectedValidation.id, comment: rejectComment });
                }
              }}
