...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'orderstatus') THEN
        CREATE TYPE orderstatus AS ENUM ('PENDING_PAYMENT', 'PAYMENT_UPLOADED', 'PAYMENT_APPROVED', 'PAYMENT_REJECTED', 'PENDING', 'AWAITING_VALIDATION', 'NEEDS_ACTION', 'PENDING_DESIGNER', 'DESIGNING', 'READY_FOR_PRINT', 'PRINTING', 'PRINTED', 'SHIPPED', 'DELIVERED', 'CANCELLED');
    END IF;
